import assert from 'node:assert';
import { createPrivateKey, createPublicKey } from 'node:crypto';
import { describe, it, beforeEach, afterEach } from 'node:test';

import { generateConfigKeyPair } from '../src/helpers';
import { decryptConfigData, encryptConfigData, loadConfig } from '../src/index';

const SAMPLE_DECRYPTION_KEY =
    '';

describe('API', () => {
    beforeEach(() => {
        delete process.env.CONFIG_DECRYPTION_SECRET;
        delete process.env.CONFIG_DECRYPTION_KEY;
    });

    afterEach(() => {
        delete process.env.CONFIG_DECRYPTION_SECRET;
        delete process.env.CONFIG_DECRYPTION_KEY;
    });

    it('should decrypt and load an encrypted config', async () => {
        process.env.VAR_4 = 'VAR_4_OVERRIDE';

        const config = loadConfig({
            file: `${__dirname}/fixtures/sample.enc.env`,
            key: SAMPLE_DECRYPTION_KEY
        });

        const expected = {
            VAR_1: 'a',
            VAR_2: 'b',
            VAR_3_SECRET: 'the quick brown fox jumps over the lazy dog',
            VAR_4: 'VAR_4_OVERRIDE',
            VAR_5_SECRET: 'supercalifragilisticexpialidocious',
            BLANK_VAR: ''
        };

        for (const [key, value] of Object.entries(expected)) {
            assert.strictEqual(config[key], value, `config.${key} should equal ${JSON.stringify(value)}`);
        }
    });

    it('should load a config with no encryption', async () => {
        const config = loadConfig({
            file: `${__dirname}/fixtures/sample.env`
        });

        const expected = {
            VAR_1: 'a',
            VAR_2: 'b',
            VAR_3_SECRET: 'the quick brown fox jumps over the lazy dog',
            VAR_4: 'VAR_4_OVERRIDE',
            VAR_5_SECRET: 'supercalifragilisticexpialidocious',
            BLANK_VAR: ''
        };

        for (const [key, value] of Object.entries(expected)) {
            assert.strictEqual(config[key], value, `config.${key} should equal ${JSON.stringify(value)}`);
        }
    });

    it('should encrypt new values with the v2 key format', async () => {
        const { privateKey, publicKey } = generateConfigKeyPair();

        assert.ok(publicKey.length < 100, 'public key should be short');
        assert.ok(privateKey.length < 100, 'private key should be short');

        const encrypted = encryptConfigData(publicKey, {
            VAR_SECRET: 'the quick brown fox jumps over the lazy dog'
        });
        const payload = Buffer.from(encrypted.VAR_SECRET.substring(3, encrypted.VAR_SECRET.length - 1), 'base64');

        assert.strictEqual(payload.readUInt8(0), 2);
        assert.deepStrictEqual(decryptConfigData(privateKey, encrypted), {
            VAR_SECRET: 'the quick brown fox jumps over the lazy dog'
        });
    });

    it('should still encrypt and decrypt with legacy RSA keys', async () => {
        const privateKey = createPrivateKey({
            key: Buffer.from(SAMPLE_DECRYPTION_KEY, 'base64'),
            format: 'der',
            type: 'pkcs8'
        });
        const publicKeyText = createPublicKey(privateKey)
            .export({
                type: 'spki',
                format: 'der'
            })
            .toString('base64')
            .replace(/=+$/, '');

        const encrypted = encryptConfigData(publicKeyText, {
            VAR_SECRET: 'legacy RSA should keep working'
        });
        const payload = Buffer.from(encrypted.VAR_SECRET.substring(3, encrypted.VAR_SECRET.length - 1), 'base64');

        assert.strictEqual(payload.readUInt8(0), 1);
        assert.deepStrictEqual(decryptConfigData(SAMPLE_DECRYPTION_KEY, encrypted), {
            VAR_SECRET: 'legacy RSA should keep working'
        });
    });

    it('should throw if the config is encrypted and no key is provided', async () => {
        assert.throws(
            () => {
                loadConfig({
                    file: `${__dirname}/fixtures/sample.enc.env`
                });
            },
            { message: /No decryption key was provided/ }
        );
    });

    it('should use CONFIG_DECRYPTION_SECRET as the default decryption key', async () => {
        process.env.CONFIG_DECRYPTION_SECRET = SAMPLE_DECRYPTION_KEY;

        const config = loadConfig({
            file: `${__dirname}/fixtures/sample.enc.env`
        });

        assert.strictEqual(config.VAR_3_SECRET, 'the quick brown fox jumps over the lazy dog');
        assert.strictEqual(config.CONFIG_DECRYPTION_SECRET, undefined);
        assert.strictEqual(process.env.CONFIG_DECRYPTION_SECRET, undefined);
    });

    it('should fall back to CONFIG_DECRYPTION_KEY as the default decryption key', async () => {
        process.env.CONFIG_DECRYPTION_KEY = SAMPLE_DECRYPTION_KEY;

        const config = loadConfig({
            file: `${__dirname}/fixtures/sample.enc.env`
        });

        assert.strictEqual(config.VAR_3_SECRET, 'the quick brown fox jumps over the lazy dog');
        assert.strictEqual(config.CONFIG_DECRYPTION_KEY, undefined);
        assert.strictEqual(process.env.CONFIG_DECRYPTION_KEY, undefined);
    });

    it('should prefer CONFIG_DECRYPTION_SECRET over CONFIG_DECRYPTION_KEY', async () => {
        process.env.CONFIG_DECRYPTION_SECRET = SAMPLE_DECRYPTION_KEY;
        process.env.CONFIG_DECRYPTION_KEY = 'not-a-valid-private-key';

        const config = loadConfig({
            file: `${__dirname}/fixtures/sample.enc.env`
        });

        assert.strictEqual(config.VAR_3_SECRET, 'the quick brown fox jumps over the lazy dog');
        assert.strictEqual(config.CONFIG_DECRYPTION_SECRET, undefined);
        assert.strictEqual(config.CONFIG_DECRYPTION_KEY, undefined);
    });
});
